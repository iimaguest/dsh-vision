...
import { viewPresentationMeta, fsObservedValueFields } from "./fs-observed-meta.js";
 ...
 */
export const DEFAULT_VIEW_PROMPT = 'Describe this image for a coding assistant. Transcribe visible text. Name layout, errors, and coordinates when they matter.';
/**
 ...
 */
export function resolveViewPrompt(question) {
    if (question === undefined)
        return DEFAULT_VIEW_PROMPT;
 ...
 */
export function formatViewOutput(value) {
 ...
 */
export function presentViewCall(args) {
    return {
        card: 'generic',
 ...
 */
export function applyViewImageTool(ctx, timeoutMs) {
        ...
        async execute(args, exec) {
            ...
            try {
                ...
                if (error.code === 'IMAGE_DIMENSION_TOO_LARGE') {
                    throw new Error(`cannot view "${target.displayPath}": at least one image side exceeds the ${attachments.imageLimits.maxImageDimension}px limit; downscale the image and view the smaller copy`, { cause: error });
                }
                if (error.code === 'IMAGE_TOO_MANY_PIXELS') {
                    throw new Error(`cannot view "${target.displayPath}": the image exceeds the ${attachments.imageLimits.maxImagePixels}-pixel decoded-size limit; downscale the image and view the smaller copy`, { cause: error });
                }
                if (error.code === 'IMAGE_TOO_LARGE') {
                    throw new Error(`cannot view "${target.displayPath}": the image cannot be stored within the deployment's byte limits; downscale the image and view the smaller copy`, { cause: error });
                }
                if (error.code === 'ATTACHMENT_WRITE_FAILED' && /16-bit PNG/iu.test(error.message)) {
                    throw new Error(`cannot view "${target.displayPath}": the 16-bit PNG could not be converted to the normalized 8-bit sRGB form; convert it to an 8-bit PNG/JPEG/WebP and retry`, { cause: error });
                }
                if (error.code !== 'IMAGE_TYPE_MISMATCH')
                    throw error;
                const extension = extname(target.displayPath).toLowerCase();
                throw new Error(`cannot view "${target.displayPath}": the ${extension} extension declares ${mediaType}, but the bytes use a different image format; rename the file to match its actual format if it is PNG/JPEG/WebP/GIF, or convert it to one of those formats`, { cause: error });
            }
